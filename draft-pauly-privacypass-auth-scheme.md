...
Privacy Pass tokens are unlinkable message authenticators that can be used to
...
solving a CAPTCHA) without allowing the relying party redeeming the token to
link client's token with the corresponding issuance flow.
...
origin is used as a replacement for CAPTCHAs, as exemplified by the original
...
The Token authenticator value is computed over the token_type, nonce, context, and
key_id fields.
