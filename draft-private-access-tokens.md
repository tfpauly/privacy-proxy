...
1. Client: requests a Private Access Token from an Issuer and relays it to a
   Redeemer for access to the Redeemer's service.
...
1. Issuer: issues Private Access Tokens to an anonymized Client on behalf of a
   Redeemer. Enforces the Redeemer's policy.

1. Redeemer: verifies any Private Access Token sent by a Client and enables
...
1. A Mediator enforces and maintains a mapping between Client identifiers and
   Client-anonymized Redeemer identifiers;

1. An Issuer enforces the Redeemer's policies based on the received
   Mediator-anonymized Client identifier and Redeemer identifier, without
   learning the Client's true identity; and

1. A Redeemer provides access to content or services to a Client upon verifying
   the Client's Private Access Token, since the verification demonstrates that
   the Client access meets the Redeemer's policies.


The Mediator, Issuer, and Redeemer each have partial knowledge of the Client's
...
(ISSUER_POLICY_WINDOW). The Mediator does not know the identity of the Redeemer
the Client is trying to access (ORIGIN_ID), but knows a Client-anonymized
identifier for it (ANON_ORIGIN_ID).

The Issuer knows the Redeemer's identity (ORIGIN_ID), and the Redeemer's policy
about client access, but only sees a Mediator-anonymized Client identifier
(ANON_CLIENT_ID). Issuers know the Redeemer's policies and enforce them on
behalf of the Redeemer. An example policy is: "Limit 10 accesses per Client".
More examples and their use cases are discussed in {{examples}}.

The Redeemer receives a Private Access Token from the client. Verification of
this token demonstrates to the Redeemer that the Client meets its policies
