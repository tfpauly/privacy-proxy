...
The Client then constructs a Private Access Token request using ORIGIN_ID and blinded_req,
encoded using TLS notation from {{!TLS13=RFC8446}}, Section 3:

~~~
struct {
  opaque anon_origin_id[32];
...
The Client includes the "Sec-CH-TokenOrigin" header in this request, whose value is
...
content-length = Nk
Sec-CH-TokenOrigin = https://example.com
...
HTTP message. The Client includes the "Sec-CH-AnonymousOriginID" header in this request,
whose value is ORIGIN_ID. Finally, the Client sends this encapsulated request to the
...
content-length = ...
Sec-CH-AnonymousOriginID = ORIGIN_ID
...
for the given Client. See {{client-id}} for details of its computation.

Before forwarding the Client's encapsulated request to the Issuer, the Mediator includes a header
listing the ANON_CLIENT_ID, "Sec-CH-AnonymousClientID". An example request is shown below.
...
to the Mediator. If decapsulation succeeds, the Issuer checks for the "Sec-CH-TargetOrigin" header.
If absent, the Issuer aborts and returns a 400 error to the Mediator. If present, the Issuer proceeds
extracts ANON_CLIENT_ID from the "Sec-CH-AnonymousClientID" header, ORIGIN_NAME from the "Sec-CH-TargetOrigin"
header, and ANON_ORIGIN_ID from AccessTokenRequest.anon_origin_id, and proceeds as follows.
