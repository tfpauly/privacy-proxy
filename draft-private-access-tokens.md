...
: An identifier that is generated by Issuer based on an ISSUER_SECRET that is
...
failures until policy window passes.

TODO: Chris to describe how CLIENT_ID should be calculated
...
Issuers maintain a stable ISSUER_SECRET that they use in calculating values returned
to the Mediator. If this value changes, it will open up a possibility for Clients to
request extra tokens for an Origin without being limited, within a policy window.

TODO: Chris to describe how ISSUER_SECRET should be calculated
...
The Client generates a random 32-byte nonce for this specific request, "request_nonce".
It combines this with CLIENT_ID to generate "request_tag".

~~~
request_tag = HashToGroup(CLIENT_ID) * request_nonce
~~~
TODO: Chris to update this

The Client then constructs a Private Access Token request using blinded_req and
request_tag:

~~~
struct {
    uint8_t version;
    uint8_t request_tag[32];
...
- "request_tag" is calculated as described above.
...
The Mediator finally checks to ensure that the AccessTokenRequest.request_tag value
matches its own calculation of request_tag = HashToGroup(CLIENT_ID) * request_nonce.
If the request_tag does not match, the Mediator rejects the request with an HTTP
400 error.
...
The Issuer also generates a value, "origin_tag", as follows:

~~~
origin_tag = HashToScalar(ORIGIN_NAME) * ISSUER_SECRET * request_tag
~~~
...
origin_tag set in the "Origin-Tag" header.
...
"Origin-Tag" header, and uses the value to determine ANON_ISSUER_ORIGIN_ID.

~~~
ANON_ISSUER_ORIGIN_ID = origin_tag / request_nonce
~~~

If the "Origin-Tag" is missing, or if the same ANON_ISSUER_ORIGIN_ID was previously
